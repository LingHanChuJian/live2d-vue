...
//     logError(`file: ${url} <br>line ${line} ${msg}`)
// }
...
function modelTurnHead(event) {
    ...
    if (LAppDefine.DEBUG_MOUSE_LOG)
        logInfo(`onMouseMove device( x: ${event.clientX} y: ${event.clientY} ) view( x: ${vx} y: ${vy} )`)
...
function modelTapEvent(event) {
    ...
    if (LAppDefine.DEBUG_MOUSE_LOG)
        logInfo(`onMouseDown device( x: ${event.clientX} y: ${event.clientY} ) view( x: ${vx} y: ${vy})`)
